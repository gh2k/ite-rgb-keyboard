...
if (argv.length == 0) {
  ...
  console.log("  # node index.js color 0 0-18 red               red on all row 0 keys");
  console.log("  # node index.js color 0,5 0-18 red \\           red on rows 0 and 5");
  console.log("       color 2-4 0-20 white                      and white on rows 2 to 4");
...
k.initialize(() => {
  ...
  for (var c = 0; c < argv.length; c++) {
    if (argv[c] === 'mode') {
      ...
      let rows = getrange(argv[++c]);
      let cols = getrange(argv[++c]);
      let color = argv[++c];
